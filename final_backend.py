...
def generate_pdf_report(
    ...
    # Title
    story.append(Paragraph("<b>FitPulse – Fitness Analytics Report</b>", styles["Title"]))
    ...
    table_data = [
        ["Metric", "Value"],
        ["Average Heart Rate", f"{stats['avg_hr']:.1f} bpm"],
        ["Average Daily Steps", f"{int(stats['avg_steps'])}"],
        ["Average Sleep", f"{stats['avg_sleep']:.1f} hrs/night"],
        ["Health Label", stats["label"]],
    ]
    ...
    # Suggestions
    story.append(Paragraph("<b>Fitness Recommendations</b>", styles["Heading2"]))
    for s in suggestions:
        story.append(Paragraph(f"- {s}", styles["Normal"]))
    story.append(Spacer(1, 14))

    # AI Narrative
    story.append(Paragraph("<b>AI Insight Summary</b>", styles["Heading2"]))
...
async def generate_report(user_id: str):
    ...
    insights = await run_in_threadpool(_generate_insights_job, user_id)

    # Ask AI for narrative summary
    ai_prompt = f"""
    Generate a concise professional fitness report summary for User {user_id}.
    Base it only on these observations:
    {insights}
    """

    ai_text = llm.invoke(ai_prompt).content

    # Date range
    df = CLEAN_DF[CLEAN_DF["user_id"] == int(user_id)]
    date_range = f"{df['timestamp'].min().date()} to {df['timestamp'].max().date()}"

    # Temp PDF
    temp_dir = tempfile.gettempdir()
    pdf_path = os.path.join(temp_dir, f"FitPulse_Report_User_{user_id}.pdf")
    ...
    return FileResponse(
        ...
        filename=f"FitPulse_Report_User_{user_id}.pdf"
    )
